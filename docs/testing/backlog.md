...
| TST-059F | XPS-100 CONFIG_3 R11 command contract | Schema / Native C++ unit | Done | Opt-in R11 control preserves every template byte except byte 10 and checksum, and rejects unsafe requests | Tests pin the observed 35 C packet and expected 40 C `0x8C`/`0xA1` bytes; control requires PC1001 detection and accepts only 35.0-40.0 C half steps |
...
| TST-061A | XPS-100 R11 active echo | Manual hardware-in-the-loop | Planned | Supervised 35-to-40-to-35 C writes echo exactly and preserve all neighboring CONFIG_3 bytes | Keep Active Mode off before/after; do not test above 40 C |
