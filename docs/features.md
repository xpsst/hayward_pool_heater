...
| INTEGRATION-087 | Optional Loxone HTTP bridge | Done | Loxone can poll stable numeric climate/status fields and request OFF/HEAT/COOL/AUTO or target changes | `web_ui.loxone_api` defaults off, uses the ESPHome web-server authentication, cannot enable Active Mode, and returns explicit passive/offline errors |
| HIL-088 | Loxone active-mode commissioning | In Progress | Supervised target and HEAT control are proven; OFF, COOL, and return to AUTO still need one-at-a-time live confirmation | Follow `docs/loxone-integration.md`; do not hand PV automation control to the endpoint until this gate is complete |
